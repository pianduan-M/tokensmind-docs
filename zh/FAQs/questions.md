...
### tokensmind 是否会存储用户 API 请求内容？

- tokensmind 不会存储您通过 API 发送的任何请求内容，也不会记录模型返回的响应内容。 tokensmind 的角色仅为中转通道，用于将您的请求安全地转发至相应的模型提供商，并将对方的响应原样返回给您。
...
这些附加内容来自工具端，而非 tokensmind 平台生成。
...
针对 Claude 模型， tokensmind 当前支持两种调用方式：
...
- OpenAI 标准端点：https://tokensmind.ai/v1 （支持 GPT 及兼容模型）
- Claude 自动转发端点：https://tokensmind.ai （兼容 Anthropic SDK 调用方式）
...
- tokensmind 自身不会保留具体请求数据，但若底层云厂商（如模型提供方或托管平台）出于安全或合规要求记录访问日志，该部分数据将受其隐私政策约束。
...
可通过 tokensmind 控制台查看调用量、Token 消耗量及计费明细。
